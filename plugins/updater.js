...
cmd({  
  ...
  try {  
    await reply("```🔍 Checking for JBKENY  updates...```\n");  
      
    // Get latest commit from GitHub  
    const { data: commitData } = await axios.get("https://api.github.com/repos/jacobmusy/JBKENY/commits/main");  
    ...
    if (latestCommitHash === currentHash) {  
      return reply("```✅ Your JBKENY bot is already up-to-date!```\n");  
    }  

    await reply("```JBKENY Bot Updating...🚀```\n");  
      
    // Download latest code  
    const zipPath = path.join(__dirname, "latest.zip");  
    const { data: zipData } = await axios.get("https://github.com/Jacobmusy/JBKENY/archive/main.zip", { responseType: "arraybuffer" });  
